  ...
  return (
    ...
    >
      <div className="glass-card__header">
        ...
        <div>
          <div className="glass-card__title">Baseline vs Production Dataset Comparison</div>
          <div className="glass-card__subtitle">Comprehensive delta analysis across all features</div>
        </div>
      </div>
      ...
      <div className="stats-row" style={{ marginTop: 0, marginBottom: 20 }}>
        <div className="stat-item">
          <div className="stat-item__label">Matched Features</div>
      ...
      <div style={{ fontSize: 13, color: "var(--text-secondary)", fontWeight: 600, marginBottom: 12 }}>
        Per-Feature Quality & Missingness Delta:
      </div>
            ...
            >
              <div className="drift-feature__header">
                <div className="drift-feature__name">
                  ...
                  {hasDrift && (
                    <span className="drift-badge drift-badge--drift">DRIFT DETECTED</span>
                  )}
                </div>
                {change !== 0 && (
                  <div
                  ...
                  >
                    {change > 0 ? "↑ +" : "↓ "}{change.toFixed(1)}% missingness delta
                  </div>
                )}
              </div>
              ...
              <div className="drift-feature__metrics">
                <div>
                  <div className="drift-feature__metric-label">Reference Baseline</div>
                  <div className="drift-feature__metric-value">
                    ...
                    <span style={{ color: "var(--text-dim)", marginLeft: 6, fontSize: 11 }}>
                      ({item.reference.missing_count.toLocaleString()} nulls)
                    </span>
                  </div>
                </div>
                <div>
                  <div className="drift-feature__metric-label">Current Production</div>
                  <div className="drift-feature__metric-value">
                    {item.current.missing_percent.toFixed(2)}%
                    <span style={{ color: "var(--text-dim)", marginLeft: 6, fontSize: 11 }}>
                      ({item.current.missing_count.toLocaleString()} nulls)
                    </span>
                  </div>
                </div>
              </div>
              ...
              {item.drift_info?.drift_score != null && (
                <div className="drift-feature__score">
                  PSI: <strong>{item.drift_info.psi?.toFixed(4)}</strong> · KS p-value: <strong>{item.drift_info.ks_p_value?.toFixed(4)}</strong> · JS Divergence: <strong>{item.drift_info.js_divergence?.toFixed(4)}</strong>
                </div>
              )}
            </motion.div>
          );
      ...
      {drift?.drift_summary && (
        <div className={`drift-summary ${drift.drift_detected ? "drift-summary--danger" : "drift-summary--success"}`}>
          <div className="drift-summary__title">
            {drift.drift_detected ? "⚠️ Quality & Drift Advisory" : "✓ Quality Summary"}
          </div>
